...
```bash
sudo chown -R wuyuhang:wuyuhang ~/openEuler-5.10.0-136-src
```
...
你前面从服务器 ISO 中“榨取”了 `.config`，把最终确认可用的 `.config` 放到源码根目录：

```bash
cp ~/ft-kernel/boot/config-5.10.0-136.12.0.86.aarch64 .config
```
...
在 `device driver` 里面把 `NVMe` 编译到内核中，而不是模块。

---

在根目录的 Makefile 中加上后缀
