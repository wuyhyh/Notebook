好的，下面是你需要的完整总结，适用于 **Fedora 系统启用 BLS（BootLoaderSpec）模式**的情况，流程涵盖：
...
```
title Fedora Linux (6.14.0-63.fc42.x86_64)
title Fedora Linux (6.16.0+)
```
...
| 任务         | 命令                                    |
|------------|---------------------------------------|
| 查看启动项      | `ls /boot/loader/entries/`            |
| 设置默认启动     | `grub2-set-default <entry>`           |
| 设置临时启动     | `grub2-reboot <entry>`                |
| 删除启动项（自编译） | `rm /boot/loader/entries/*6.xx*.conf` |
| 删除内核镜像文件   | `rm /boot/vmlinuz-*6.xx*` 等           |
| 当前运行内核     | `uname -r`                            |

---

如你之后常做内核调试、重装、切换，我可以帮你写个一键管理脚本（自动识别、切换、清理），也可以集成内核备份和恢复逻辑。是否需要？
