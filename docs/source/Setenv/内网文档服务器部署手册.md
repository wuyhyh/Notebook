# 内网文档服务器部署手册（Rocky Linux 10 + GitLab 共存版）
...
3. Fedora/rocky 这类默认启用 SELinux：
    ...
    * 站点目录需要 `httpd_sys_content_t` 标签，否则容易出现 403（尤其是部署后文件标签不对）。旧文档里也明确要求对
      `/srv/www/notebook` 做 `semanage fcontext + restorecon`。
4. scp 直接覆盖不会删除旧文件，会产生“线上残留”。旧文档用 `rsync -a --delete` 解决这个问题。
   你现在采用“临时目录 + 原子切换 + restorecon”的发布方式，本质上也是为了解决同一类问题。
...
这一步就是你这次“只能打开首页、点进去 403”最终解决掉的原因：目录/文件标签或权限不一致。
...
你旧文档已经把这件事写得很清楚：如果每次都要输入密码，就需要做 `.ssh` 配置（ssh-keygen + ssh-copy-id）。

建议你在“实际运行发布脚本的环境”里做一次（WSL 更方便）：
...
## 附录 A：历史参考（VMware Fedora / Ubuntu 版本做法）

这部分保留你三个月前的方案作为对照，便于以后迁移到别的发行版或重新拆分部署。

### A.1 核心思想

* 站点是纯静态，服务器只需要托管 `site/`。
* 推荐 Nginx 托管静态目录 `/srv/www/notebook/`。

### A.2 Fedora 方案的关键片段

#### 构建

旧文档给的是直接 `sphinx-build -b html docs/source site`。

#### 原子发布 + 清残留（rsync --delete）

旧文档推荐：先传 `/tmp/site-upload/`，再 `rsync -a --delete` 覆盖到 `/srv/www/notebook/`。

#### SELinux

对目录设置 `httpd_sys_content_t` 并 `restorecon`（只做一次）。

### A.3 Ubuntu 迁移要点

旧文档也给了 Ubuntu 下的等价命令，Ubuntu 默认无 SELinux，配置更省事。

---

## 附录 B：你当前仓库脚本的定位说明

* `build.sh`：生成 `site/`，依赖 `sphinx-build`；会清理旧目录并生成 sha256。
* `re_deployment.sh`：你会持续维护的“一键发布脚本”。当前仓库里那份最早版本是 `scp` 直拷贝，容易残留。
  现在请以“临时目录 + 原子切换 + restorecon”为准，避免残留与 403。
