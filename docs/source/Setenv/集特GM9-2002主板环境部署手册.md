...
先备份 `/etc/yum.repos.d/` 下的 `openEuler.repo`
...
再在 `/etc/yum.repos.d/` 目录下创建新的 `oe-local-iso.repo` 文件：
...
先使用在 windows PC 上复制 rpm 包到 U 盘 (**注意不是启动盘**，是另一个 U 盘)
...
假设这 3 个 rpm 包在 `/root`, 依次安装内核、内核头文件、内核开发依赖包

```bash
dnf install ./kernel-5.10.0-136.12.0.86.aarch64.rpm
```

```bash
dnf install ./kernel-headers-5.10.0-136.12.0.86.aarch64.rpm
```

```bash
dnf install ./kernel-devel-5.10.0-136.12.0.86.aarch64.rpm
```
...
```bash
KREL=5.10.0-136.12.0.86.aarch64
```
...
sudo nmcli con add type ethernet ifname enaphyt4i0 con-name enaphyt4i0-static \
  ipv4.method manual ipv4.addresses 192.168.11.123/24 \
...
（你怎么配我不展开了，目标是保证 PC 能 ping 到 192.168.11.123。）
