...
* 内核源码树：`~/openEuler-5.10.0-216-src`
...
```bash
cd /mnt/newroot
sudo tar --numeric-owner -xpf ./oe-rootfs-${KREL}.tar
```

```text
setenv bootargs 'console=ttyAMA1,115200 root=/dev/nvme0n1p2 rw rootwait earlycon ignore_loglevel loglevel=8 audit=0'
```

```text
HASH='$6$izt.cZpJqQvq6ugi$pCxsksDaB2NTPKcJ3SPWC3r/el67VxFLeVqkE3uJMhMnUYkR3gzm8LzIbK04eFfQAKHbyUWmIOfc7NgPA4EKP/'
sed -i "s|^root:[^:]*:|root:${HASH}:|" /mnt/nvme_p2/etc/shadow
sync
...
setenv loadimage_nvme2 'ext4load nvme 0:2 ${kernel_addr_r} /boot/Image-5.10.0-Dec-24th'
setenv loaddtb_nvme2   'ext4load nvme 0:2 ${fdt_addr_r} /boot/pd2008-devboard-dsk.dtb'
```
...
```bash
setenv bootcmd_nvme2 'run loadimage_nvme2 loaddtb_nvme2; \
setenv bootargs ${bootargs_nvme2}; \
