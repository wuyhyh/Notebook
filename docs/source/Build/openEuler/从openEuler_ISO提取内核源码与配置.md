...
cd ~/oeiso
ls
# 可以看到类似：EFI  Packages  RPM-GPG-KEY-openEuler  TRANS.TBL  boot.catalog  docs  images  repodata
```
...
...
kernel-rt-*.rpm / kernel-rt-source-*.rpm  # RT 内核，可选
```
...
```bash
mkdir -p ~/oe-kernel
cp kernel-source-5.10.0-216.0.0.115.oe2203sp4.aarch64.rpm \
   kernel-5.10.0-216.0.0.115.oe2203sp4.aarch64.rpm \
   ~/oe-kernel/
```
...
```text
usr/src/kernels/5.10.0-216.0.0.115.oe2203sp4.aarch64/
```

把源码树移到一个单独路径：

```bash
mv usr/src/kernels/5.10.0-216.0.0.115.oe2203sp4.aarch64 \
...
cp ~/oe-kernel/boot/config-5.10.0-216.0.0.115.oe2203sp4.aarch64 .config
```
