...
```shell
sudo dnf install git -y # 在 Fedora 上安装 Git
sudo apt install git -y # 在 Ubuntu 上安装 Git
```
...
配置信息保存在文件`.gitconfig下`。子目录的配置会覆盖顶层目录的配置。

### 设置`SSH`访问`github`

- 在本地操作系统的`~/.ssh`目录下生成公钥和私钥

```shell
ssh-keygen
```

- 复制公钥`isa_pub`到 `github` 端
...
#### 查看当前工作区的修改与上一次`commit`的区别：

```shell
git diff
git diff --cached # 对比 暂存区 和 commit 区的区别
```
...
如果你已经把`commit`推送到了远端仓库，那么再次推送，要加上`force`，但是非常**危险**，为了避免覆盖远程仓库中其他人的工作，使用
`--force-with-lease`

```shell
git push <remote_rep_name> <remote_branch_name> -f
git push <remote_rep_name> <remote_branch_name> --force-with-lease
```
...
`fetch`只会取回本地没有的数据，不会修改本地工作目录下的数据。
...
# 为本地分支设置一个远程跟踪分支
git branch --set-upstream-to=<remote_rep>/<remote_branch> 
```
