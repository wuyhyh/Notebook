...
```text
mv Image*phytium*.bin Image-5.10.bin
mv pd2008*.dtb pd2008.dtb
```
...
```text
setenv bootargs "console=ttyAMA1,115200 audit=0 earlycon=pl011,0x28004000 noinitrd root=/dev/nvme0n1p1 rootwait rw arm64.nolse=1"
```
...
1. 编译系统如何定制化内核？
2. 网络驱动如何加载
3. 如何启用网络 bonding，两路网络外部都有数据包流入时如何处理？
4. 制作 ramdisk 启动小型 Linux


```text
tftpboot 0x90100000 Image
tftpboot $fdt_addr_r u-boot-d2000-devboard-dsk.dtb
tftpboot 0x93000000 initramfs.cpio.gz
```
```text
setenv bootargs 'console=ttyAMA1,115200 earlycon=pl011,0x28001000 root=/dev/ram0 rw'
```

```text
booti 0x90100000 0x93000000:$filesize 0x90000000
```

```text
# TFTP 三件套
tftpboot 0x90100000 Image
tftpboot 0x90000000 pd2008.dtb
tftpboot 0x93000000 rootfs.cpio.gz

# 指定在 initramfs 里跑的 init（Buildroot 默认 /sbin/init；调试可用 /bin/sh）
setenv bootargs 'console=ttyAMA1,115200 earlycon=pl011,0x28001000 rdinit=/sbin/init'

# 注意：filesize 是“上一次 tftp”的大小，这里就是 cpio.gz 的大小
setexpr rdsize $filesize
booti 0x90100000 0x93000000:$rdsize 0x90000000
```